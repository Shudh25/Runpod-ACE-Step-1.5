...
# =============================================================================
# Stage 3 — Copy to Network Volume (+ optional S3 upload)
# =============================================================================

def _upload_to_s3(filepath: str, s3_config: dict, s3_key: str) -> str:
    """
    Upload a file to S3-compatible storage using AWS Signature V4.
    Uses only stdlib — no boto3 dependency required.
    Returns the S3 key on success.

    s3_config keys:
      endpoint   — e.g. "https://s3.amazonaws.com" or custom endpoint
      bucket     — bucket name
    ...
    _log("S3Upload", f"  ✓ {s3_key} uploaded")
    return s3_key


def _stage3_save(lora_weights_dir: str, lora_name: str, s3_config: dict = None) -> tuple[str, list]:
    """
    Copy LoRA weights from temp working dir to the Network Volume.
    If s3_config is provided, also uploads each file to S3 storage.
    ...
    if s3_config and s3_config.get("endpoint"):
        _log("Stage3", f"Uploading {len(file_list)} file(s) to S3...")
        for rel_path in file_list:
            src    = str(dest / rel_path)
            s3_key = f"{lora_name}/{rel_path}"
            try:
                _upload_to_s3(src, s3_config, s3_key)
            except Exception as exc:
                # Log and continue — local volume copy already succeeded
                _log("Stage3", f"  ✗ S3 upload failed for {s3_key}: {exc}")
    else:
        _log("Stage3", "No s3_config provided — skipping S3 upload")
...
def handler(job: dict) -> dict:
    ...
    try:
        ...
        # ── Stage 3: Save to Volume (+ optional S3 upload) ───────────────────
        _log("Handler", "=== Stage 3: Save to Network Volume ===")
        s3_config = inp.get("s3_config")   # optional — omit to skip S3 upload
        dest_path, file_list = _stage3_save(lora_weights_dir, lora_name, s3_config=s3_config)
